...
from unirec.utils.general import dict2str
...
class Solver(Trainer):
    ...
    def fit(self, train_data, valid_data=None, save_model=False, load_pretrained_model=False, model_file=None, verbose=2):
        logger = self.logger
        ...
        if valid_data: 
            ...
            valid_score_output = ("evaluating" + " [time" + ": %.2fs, " + "%s: %f]") % \
                                    (valid_end_time - valid_start_time, self.key_metric, valid_score)
